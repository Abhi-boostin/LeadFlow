import { PrismaClient, LeadStatus, DiscussionSource } from '@prisma/client';
...
async function main() {
  ...
  async function lead(input: {
    ...
    if (!latest) throw new Error(`Lead ${input.name} needs at least one discussion`);
    ...
    // Insert chronologically so the order in the timeline is natural.
  ...
  // 1. Sarah Connor (Acme Corp) - PROPOSAL_SENT - follow-up today 2pm.
  // Matches the wireframe's pinned "today's follow-ups" card.
  await lead({
    name: 'Sarah Connor',
    company: 'Acme Corp',
    phone: '555-0199',
  ...
  // 2. Hank Scorpio (Globex) - NEW - inbound from web form 2 hours ago.
  await lead({
    name: 'Hank Scorpio',
    company: 'Globex',
    phone: '555-0102',
    status: LeadStatus.NEW,
    discussions: [{ note: 'Inbound lead from website contact form.', createdAt: hoursAgo(2) }],
  });

  // 3. Bill Lumbergh (Initech) - CONTACTED - last touch 1 week ago.
  await lead({
    name: 'Bill Lumbergh',
    company: 'Initech',
    phone: '555-0173',
  ...
  // 4. Bruce Wayne (Wayne Ent) - WON - long history, closed 3 weeks ago.
  await lead({
    name: 'Bruce Wayne',
    company: 'Wayne Ent',
    phone: '555-0118',
  ...
  // 5. Hermione Granger (Hogwarts Inc) - QUALIFIED - OVERDUE follow-up by 2 days.
  // Tests the red highlighting for overdue.
  await lead({
  ...
  // 6. Tony Stark (Stark Industries) - LOST - decided to build internal.
  await lead({
    name: 'Tony Stark',
  ...
  // 7. Diana Prince (Themyscira Imports) - NEW - just landed.
  await lead({
    name: 'Diana Prince',
  ...
  // 8. Clark Kent (Daily Planet) - CONTACTED - follow-up tomorrow 10am.
  await lead({
    name: 'Clark Kent',
